...
impl Redirect {
    ...
    #[instrument(kind = Gauge, name = "o3.conns")]
    ...
    async fn handle_quic_stream(quic_stream: QuicStream, redirect_to: SocketAddr) -> Result<()> {
        ...
        static GAUGE_FORWARD: std::sync::LazyLock<Option<metricrs::Gauge>> =
            std::sync::LazyLock::new(|| {
                metricrs::global::get_global_registry().map(|registry| {
                    use metricrs::*;
                    registry.gauge(Token::new("o3.forward.pipelines", &[]))
                })
            });
        ...
        static GAUGE_BACKWARD: std::sync::LazyLock<Option<metricrs::Gauge>> =
            std::sync::LazyLock::new(|| {
                metricrs::global::get_global_registry().map(|registry| {
                    use metricrs::*;
                    registry.gauge(Token::new("o3.backward.pipelines", &[]))
                })
            });

        tokio::spawn(async move {
            log::info!("forward, from={}, to={}", quic_stream_read, redirect_to);
        ...
        tokio::spawn(async move {
            log::info!("backward, from={}, to={}", redirect_to, quic_stream_send);
